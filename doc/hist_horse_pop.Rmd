...
```{r max horse table, message=FALSE, warning=FALSE}
 max_horses <- horse_pop %>% 
  ...
  summarize(Maximum = max(Value),
            Minimum = min(Value))
kable(max_horses, caption = "Table 1. This is a summary of the horse population for each provinces between 1940-1972")
```

Below we zoom in and look at the province of `r max_horses$Province[1]`:

```{r plot province, fig.width=5, fig.height=4, fig.cap="Figure 2. Horse populations for province  of focus", out.width='40%'}
